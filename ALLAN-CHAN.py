...
def _f_t(y, epsilon):
    return (1./np.power(epsilon,2.))*(y - tf.pow(y, 3))

def initialization_values(num_sample):
    n_1 = np.array([1, 0])
    n_2 = np.array([0, 1])
    n_=np.ones(d)
    lato=200
    n_mezzi=int(np.sqrt(num_sample))
    _x_init = np.ones([num_sample, d])
    for i in range(0, n_mezzi):
        a=(50.*i/1.)-(lato*0.5)
        for j in range(0, n_mezzi):
            b = (50*j/1.)-(lato*0.5)
            _x_init[i*n_mezzi+j,:] = (n_*n_1*a)+(n_*n_2*b)
    return _x_init

def initialization_values_test(num_sample):
    n_1 = np.array([1, 0])
    n_2 = np.array([0, 1])
    n_=np.ones(d)
    lato=200
...
def phi (x, T):
    lambda_0 = 200
    delta_0 = 0.01
    return delta_0*tf.reduce_prod(tf.sin(np.pi*2.*x/lambda_0), 1, keepdims=True) 
...
def _approximate():
    ...
    t1_train = time.time()
    file_out.write('% i \t '' % f\t % f\t % f\t %f \n' % (gs, lr, t1_train - t0_train, _loss, _u_0[0]))
...
with tf.compat.v1.Session() as sess:  # TensorFlow's session starts
    ...
    file_name = 'ExampleEM_' + str(sample) + '.txt'
    file_name1 = 'U0EM_' + str(sample) + '.txt'
    file_name2 = 'UtEM_' + str(sample) + '.txt'
    T = float(sys.argv[2])
    epsilon=0.5
    N, d = 20, 2 # T is the final time, N is the number of the step we make for each trajectory, d is the number of dimensions
    dt = T / N # dt is the step interval that we need to use in the integration
    batch_size = 16 # this is the size for each batch
    batch_test = 10000 # this is the size for each batch
    ...
    train_steps = 10000# this is the number of training steps
...
#lr_values = [1., 0.1, 0.01, 0.001] # learning values #old
    ...
    with tf.variable_scope('forward'):
      ...
      #  g_s_i = 'forward' + str(0)
        ...
        #u_0 = tf.Variable(tf.random.uniform([1], minval=0.3, maxval=0.6, dtype=dtype), name='u_0')#  dovrebbe essere da 40 a 50
        ...
        for t in range(0, N - 1):
            g_s_t = str(t + 1)
            u_approx = u_approx - dt * _f_t(u_approx, epsilon) + tf.reduce_sum(grad_u_t * DW[:, :, t], 1, keepdims=True)
            grad_u_approx_t = _subnetwork(DX[:, :, t + 1], neurons, g_s_t)/d
            grad_u_t = grad_u_approx_t
            U.append(u_approx)
        u_approx = u_approx - dt * _f_t(u_approx, epsilon) + tf.reduce_sum(grad_u_t * DW[:, :, -1], 1, keepdims=True)
        U.append(u_approx)
    ...
    delta = u_approx - phi(phi_final, T)
    ...
    file_out1 = open(file_name1, 'a')
    file_out2 = open(file_name2, 'a')
    
    #test set, i.e. we create 4096 trajectories
    x_init_sample_ipercube=initialization_values(batch_size)
    x_test=initialization_values_test(batch_test)
    dw_valid, x_valid = integrate(batch_test, x_test)
    ...
    for step in range(train_steps + 1):
        ...
        dw_train, x_train = integrate(batch_size, x_init_sample_ipercube)
    ...
    _u_0 = sess.run([u_0], feed_dict=feed_dict_valid)
    sss = np.asarray(_u_0)
    for i in range(0, batch_test):
        file_out1.write('% f \t % f  \t  % f  \n' % (x_test[i, 0], x_test[i, 1], sss[0, i, 0]))
        if((i+1) % 5 == 0):
            file_out1.write('\n')
