...
def _f_t(y, dy):
    return ((sigma * sigma * y) - (1./d) - (sigma * sigma * 0.5)) * tf.reduce_sum(dy, 1, keepdims=True)
...
def exact_sol(x):
    return np.exp(np.sum(x))/(1. + np.exp(np.sum(x)))
...
def phi(x):
   return tf.exp(T+tf.reduce_sum(x, 1, keepdims=True))/(1. + tf.exp(T+tf.reduce_sum(x, 1, keepdims=True)))
...
def _approximate():
    x=np.zeros([d])
    s=exact_sol(x)
    lr, gs, _loss, _u_0, __l = sess.run([learning_rate, global_step, loss, u_0, ____l], feed_dict=feed_dict_valid)
    t1_train = time.time()
    file_out.write('% i\t '' % f \t % f \t % f \t %f \t %f \t %f \n' % (gs, lr, t1_train - t0_train, _loss, _u_0[0], s, __l[0]))
...
with tf.compat.v1.Session() as sess:  # TensorFlow's session starts
    ...
    MC=10000
    T, N = 0.01, 40 # T is the final time, N is the number of the step we make for each trajectory
    ...
    train_steps = 25000  # this is the number of training steps
    mc_freq = 100  # global variable used for printing
    lr_boundaries = [2500, 5000, 6000, 7000, 10000]  # defines when we want to change the learning value
    #lr_boundaries = [4000, 6000]  # defines when we want to change the learning value
    #lr_values = [0.005,0.001, 0.0005]  # learning values
    lr_values = [0.5, 0.1, 0.01, 0.001, 0.0001, 0.00001]  # learning values
    ...
    # INTEGRATION FOR u_t*
        ...
        for t in range(0, N - 1):
            ...
            g_s_t = str(t + 1)
            u_approx = u_approx - dt * _f_t(u_approx, grad_u_t) + tf.reduce_sum(grad_u_t * DW[:, :, t], 1, keepdims=True)
            grad_u_approx_t = _subnetwork(DX[:, :, t + 1], neurons, g_s_t) / d
            grad_u_t = grad_u_approx_t
        
        
        u_approx = u_approx - dt * _f_t(u_approx, grad_u_t) + tf.reduce_sum(grad_u_t * DW[:, :, -1], 1, keepdims=True)
